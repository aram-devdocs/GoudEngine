...
def generate(schema: dict) -> str:
    ...
    lines.append("pub(crate) fn register_enum_constants(lua: &Lua) -> LuaResult<()> {")
    ...
    for enum_name, enum_def in enums.items():
        ...
        if isinstance(values, dict):
            for vname, vval in values.items():
                lines.append(f'    tbl.set("{sdk_common.to_snake(vname)}", {vval}_i64)?;')
        lines.append(f'    globals.set("{snake}", tbl)?;')
