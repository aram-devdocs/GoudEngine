...
use crate::types::{NapiSpriteCmd, NapiTextCmd};
...
use goud_engine::ffi::renderer::{
    ...
    goud_renderer_draw_text, goud_renderer_draw_text_batch, FfiSpriteCmd, FfiTextCmd,
...
use goud_engine::ffi::renderer3d::{
    ...
    goud_renderer3d_create_scene, goud_renderer3d_create_sphere,
    goud_renderer3d_destroy_model, goud_renderer3d_destroy_object,
    ...
    goud_renderer3d_instantiate_model,
    ...
    goud_renderer3d_set_camera_position,
    ...
    goud_renderer3d_set_material_sorting_enabled,
    goud_renderer3d_set_object_position, goud_renderer3d_set_object_rotation,
    goud_renderer3d_set_object_scale, goud_renderer3d_set_shared_animation_eval,
    goud_renderer3d_set_skinning_mode, goud_renderer3d_stop_animation,
...
impl GoudGame {
    ...
    #[napi]
    pub fn draw_sprite_batch(&self, cmds: Vec<NapiSpriteCmd>) -> u32 {
        if cmds.is_empty() {
            return 0;
        }
        let ffi_cmds: Vec<FfiSpriteCmd> = cmds.iter().map(|c| FfiSpriteCmd {
    ...
    #[napi]
    pub fn draw_text_batch(&self, cmds: Vec<NapiTextCmd>) -> u32 {
        if cmds.is_empty() {
            return 0;
        }
        let cstrings: Vec<std::ffi::CString> = cmds.iter().map(|c| {
        ...
        let ffi_cmds: Vec<FfiTextCmd> = cmds.iter().enumerate().map(|(i, c)| FfiTextCmd {
            font_handle: c.font_handle.unwrap_or(0.0) as u64,
            text: cstrings[i].as_ptr(),
            x: c.x.unwrap_or(0.0) as f32,
            y: c.y.unwrap_or(0.0) as f32,
            font_size: c.font_size.unwrap_or(16.0) as f32,
            alignment: c.alignment.unwrap_or(0) as u8,
            direction: c.direction.unwrap_or(0) as u8,
            _pad0: 0,
            max_width: c.max_width.unwrap_or(0.0) as f32,
            line_spacing: c.line_spacing.unwrap_or(1.0) as f32,
            r: c.r.unwrap_or(1.0) as f32,
            g: c.g.unwrap_or(1.0) as f32,
            b: c.b.unwrap_or(1.0) as f32,
            a: c.a.unwrap_or(1.0) as f32,
        }).collect();
        // SAFETY: ffi_cmds and cstrings are valid for the call duration.
        unsafe {
    ...
    pub fn get_model_bounding_box(&self, model_id: u32) -> Option<Vec<f64>> {
        ...
        if ok {
            Some(vec![
                min_x as f64, min_y as f64, min_z as f64,
                max_x as f64, max_y as f64, max_z as f64,
            ])
        } else {
            None
        }
    ...
    pub fn get_animation_name(&self, model_id: u32, anim_index: i32) -> Option<String> {
        ...
        let len = unsafe {
            goud_renderer3d_get_animation_name(
                self.context_id, model_id, anim_index,
                buf.as_mut_ptr(), 256,
            )
        };
        if len < 0 {
    ...
    #[napi]
    pub fn blend_animations(&self, instance_id: u32, anim_a: i32, anim_b: i32, blend_factor: f64) -> bool {
        goud_renderer3d_blend_animations(self.context_id, instance_id, anim_a, anim_b, blend_factor as f32)
    }
    #[napi]
    pub fn transition_animation(&self, instance_id: u32, target_anim: i32, duration: f64) -> bool {
        goud_renderer3d_transition_animation(self.context_id, instance_id, target_anim, duration as f32)
    }
    #[napi]
