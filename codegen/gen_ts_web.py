...
def gen_web_wrapper():
    ...
    lines.append("  setSkinningMode(_mode: number): number { return 0; }")
    lines.append("  setMaterialSortingEnabled(_enabled: boolean): number { return 0; }")
    lines.append("  setAnimationLodEnabled(_enabled: boolean): number { return 0; }")
    lines.append("  setSharedAnimationEval(_enabled: boolean): number { return 0; }")
    ...
    lines.append("")
    # Renderer3D batch stubs (pointer-based FFI — not auto-generated by codegen)
    lines.append("  instantiateModelBatch(_sourceModelId: number, _count: number): number { return 0; }")
    lines.append("  setModelPositionsBatch(_count: number): number { return 0; }")
    lines.append("  addModelsToSceneBatch(_sceneId: number, _count: number): number { return 0; }")
    lines.append("  setModelStatic(_modelId: number, _isStatic: boolean): boolean { return false; }")
    lines.append("  setAnimationPhaseLock(_modelId: number, _enabled: boolean): boolean { return false; }")
    lines.append("  setAnimationBakingEnabled(_modelId: number, _enabled: boolean): boolean { return false; }")
    lines.append("")
    ...
    lines.append("  }")
    lines.append("")
    # Render statistics and configuration getters (not available in WASM) — return sensible defaults
    lines.append("  getActiveInstanceCount(): number { return 0; }")
    lines.append("  getAnimationEvaluationCount(): number { return 0; }")
    lines.append("  getAnimationEvaluationSavedCount(): number { return 0; }")
    lines.append("  getAnimationLodDistance(): number { return 0; }")
    lines.append("  getAnimationLodEnabled(): boolean { return false; }")
    lines.append("  getAnimationLodSkipDistance(): number { return 0; }")
    lines.append("  getBakedAnimationSampleRate(): number { return 30; }")
    lines.append("  getBoneMatrixUploadCount(): number { return 0; }")
    lines.append("  getFrustumCullingEnabled(): boolean { return false; }")
    lines.append("  getFrustumCullingFarPlane(): number { return 1000; }")
    lines.append("  getFrustumCullingFov(): number { return 60; }")
    lines.append("  getFrustumCullingNearPlane(): number { return 0.1; }")
    lines.append("  getGridAlpha(): number { return 1; }")
    lines.append("  getInstancedDrawCalls(): number { return 0; }")
    lines.append("  getInstancingEnabled(): boolean { return false; }")
    lines.append("  getMaterialSortingEnabled(): boolean { return false; }")
    lines.append("  getMinInstancesForBatching(): number { return 4; }")
    lines.append("  getShadowBias(): number { return 0.001; }")
    lines.append("  getShadowMapSize(): number { return 2048; }")
    lines.append("  getShadowsEnabled(): boolean { return false; }")
    ...
    lines.append("  setBakedAnimationSampleRate(_rate: number): number { return 0; }")
    lines.append("  setDefaultMaterialColor(_r: number, _g: number, _b: number, _a: number): number { return 0; }")
    lines.append("  setFrustumCullingFarPlane(_far: number): number { return 0; }")
    lines.append("  setFrustumCullingFov(_fovDegrees: number): number { return 0; }")
    lines.append("  setFrustumCullingNearPlane(_near: number): number { return 0; }")
    lines.append("  setGridAlpha(_alpha: number): number { return 0; }")
    lines.append("  setInstancingEnabled(_enabled: boolean): number { return 0; }")
    lines.append("  setMinInstancesForBatching(_count: number): number { return 0; }")
