...
def gen_ffi() -> None:
    ...
    # FfiSpriteCmd for batch rendering (not in schema types, hardcoded)
    lines.append("class FfiSpriteCmd(ctypes.Structure):")
    lines.append("    _fields_ = [")
    lines.append('        ("texture", ctypes.c_uint64),')
    lines.append('        ("x", ctypes.c_float),')
    lines.append('        ("y", ctypes.c_float),')
    lines.append('        ("width", ctypes.c_float),')
    lines.append('        ("height", ctypes.c_float),')
    lines.append('        ("rotation", ctypes.c_float),')
    lines.append('        ("src_x", ctypes.c_float),')
    lines.append('        ("src_y", ctypes.c_float),')
    lines.append('        ("src_w", ctypes.c_float),')
    lines.append('        ("src_h", ctypes.c_float),')
    lines.append('        ("r", ctypes.c_float),')
    lines.append('        ("g", ctypes.c_float),')
    lines.append('        ("b", ctypes.c_float),')
    lines.append('        ("a", ctypes.c_float),')
    lines.append('        ("z_layer", ctypes.c_int32),')
    lines.append('        ("_padding", ctypes.c_int32),')
    lines.append("    ]")
    lines.append("")

    # FfiTextCmd for text batch rendering (not in schema types, hardcoded)
    lines.append("class FfiTextCmd(ctypes.Structure):")
    lines.append("    _fields_ = [")
    lines.append('        ("font_handle", ctypes.c_uint64),')
    lines.append('        ("text", ctypes.c_char_p),')
    lines.append('        ("x", ctypes.c_float),')
    lines.append('        ("y", ctypes.c_float),')
    lines.append('        ("font_size", ctypes.c_float),')
    lines.append('        ("alignment", ctypes.c_uint8),')
    lines.append('        ("direction", ctypes.c_uint8),')
    lines.append('        ("_pad0", ctypes.c_uint16),')
    lines.append('        ("max_width", ctypes.c_float),')
    lines.append('        ("line_spacing", ctypes.c_float),')
    lines.append('        ("r", ctypes.c_float),')
    lines.append('        ("g", ctypes.c_float),')
    lines.append('        ("b", ctypes.c_float),')
    lines.append('        ("a", ctypes.c_float),')
