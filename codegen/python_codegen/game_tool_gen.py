...
def _emit_tool_constructor(
    ...
    *,
    ...
    if is_game:
        ...
        lines.append("    def __init__(self, config: 'ContextConfig' = None):")
        lines.append("        lib = get_lib()")
        lines.append("        self._lib = lib")
        lines.append("        if config is None:")
        lines.append("            self._ctx = lib.goud_context_create()")
        lines.append("        else:")
        lines.append("            _config_ffi = _ffi_module.GoudContextConfig()")
        lines.append("            _config_ffi.debugger.enabled = config.debugger.enabled")
        lines.append("            _config_ffi.debugger.publish_local_attach = config.debugger.publish_local_attach")
        lines.append("            _route_label_bytes = None")
        lines.append("            if config.debugger.route_label:")
        lines.append("                _route_label_bytes = config.debugger.route_label.encode('utf-8')")
        lines.append("                _config_ffi.debugger.route_label = _route_label_bytes")
        lines.append("            else:")
        lines.append("                _config_ffi.debugger.route_label = None")
        lines.append("            self._ctx = lib.goud_context_create_with_config(ctypes.byref(_config_ffi))")
        lines.append("        if self._ctx._bits == 0xFFFFFFFFFFFFFFFF:")
        lines.append("            raise RuntimeError('Failed to create headless context')")
    lines.append("")
