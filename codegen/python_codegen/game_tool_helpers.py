...
def emit_tool_method_body(
    ...
    *,
    ...
    if mname == "destroy":
        ...
        lines.append("        _kept_refs = []")
        lines.append("        for i, cmd in enumerate(cmds):")
        lines.append("            c = cmd.get('color') or Color.white()")
        lines.append("            text_bytes = (cmd.get('text', '') or '').encode('utf-8')")
        lines.append("            _kept_refs.append(text_bytes)")
        lines.append("            ffi_arr[i].font_handle = cmd.get('font_handle', 0)")
        lines.append("            ffi_arr[i].text = text_bytes")
        lines.append("            ffi_arr[i].x = cmd.get('x', 0.0)")
        lines.append("            ffi_arr[i].y = cmd.get('y', 0.0)")
        lines.append("            ffi_arr[i].font_size = cmd.get('font_size', 16.0)")
        lines.append("            ffi_arr[i].alignment = int(cmd.get('alignment', 0))")
        lines.append("            ffi_arr[i].direction = int(cmd.get('direction', 0))")
        lines.append("            ffi_arr[i]._pad0 = 0")
        lines.append("            ffi_arr[i].max_width = cmd.get('max_width', 0.0)")
        lines.append("            ffi_arr[i].line_spacing = cmd.get('line_spacing', 1.0)")
        lines.append("            ffi_arr[i].r = c.r if hasattr(c, 'r') else 1.0")
        lines.append("            ffi_arr[i].g = c.g if hasattr(c, 'g') else 1.0")
        lines.append("            ffi_arr[i].b = c.b if hasattr(c, 'b') else 1.0")
        lines.append("            ffi_arr[i].a = c.a if hasattr(c, 'a') else 1.0")
        lines.append("        return self._lib.goud_renderer_draw_text_batch(self._ctx, ffi_arr, n)")
