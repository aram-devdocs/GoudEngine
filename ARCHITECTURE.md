...
cargo build               ──► csbindgen       ──► sdks/csharp/NativeMethods.g.cs
                         └──► cbindgen        ──► codegen/generated/goud_engine.h
...
The script executes 13 ordered steps: scaffolding, build, C header validation, TypeScript bootstrap, lint-layers, validate_coverage, gen_csharp, gen_python, gen_ts_node, gen_ts_web, format, validate, and docs snippets. The validation steps abort the pipeline on failure.
