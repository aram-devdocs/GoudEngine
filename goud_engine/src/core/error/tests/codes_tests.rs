...
use crate::core::error::{
    ...
    GRAPHICS_ERROR_BASE, INPUT_ERROR_BASE, INTERNAL_ERROR_BASE, NETWORK_ERROR_BASE,
    RESOURCE_ERROR_BASE, SYSTEM_ERROR_BASE,
};
use crate::core::error::{
    ERR_DRAW_CALL_FAILED, ERR_ENTITY_NOT_FOUND, ERR_INITIALIZATION_FAILED, ERR_INTERNAL_ERROR,
    ERR_INVALID_INPUT_ACTION, ERR_INVALID_STATE, ERR_NETWORK_BIND_FAILED,
    ERR_NETWORK_CONNECT_FAILED, ERR_NETWORK_CONNECTION_NOT_FOUND, ERR_NETWORK_DISCONNECTED,
    ERR_NETWORK_INVALID_ADDRESS, ERR_NETWORK_SEND_FAILED, ERR_NETWORK_TIMEOUT,
    ERR_NOT_INITIALIZED, ERR_PLATFORM_ERROR, ERR_QUERY_FAILED, ERR_RESOURCE_NOT_FOUND,
    ERR_SHADER_COMPILATION_FAILED, ERR_WINDOW_CREATION_FAILED, SUCCESS,
};
...
fn test_error_code_ranges_are_non_overlapping() {
    ...
    assert!(SYSTEM_ERROR_BASE < NETWORK_ERROR_BASE);
    assert!(NETWORK_ERROR_BASE < INTERNAL_ERROR_BASE);
}
...
fn test_error_category_classification() {
    ...
    assert_eq!(error_category(ERR_NETWORK_BIND_FAILED), "Network");
...
fn test_error_codes_within_category_bounds() {
    ...
    assert!(ERR_NETWORK_BIND_FAILED >= 700 && ERR_NETWORK_BIND_FAILED < 800);
    assert!(ERR_NETWORK_CONNECT_FAILED >= 700 && ERR_NETWORK_CONNECT_FAILED < 800);
    assert!(ERR_NETWORK_SEND_FAILED >= 700 && ERR_NETWORK_SEND_FAILED < 800);
    assert!(ERR_NETWORK_DISCONNECTED >= 700 && ERR_NETWORK_DISCONNECTED < 800);
    assert!(ERR_NETWORK_TIMEOUT >= 700 && ERR_NETWORK_TIMEOUT < 800);
    assert!(ERR_NETWORK_INVALID_ADDRESS >= 700 && ERR_NETWORK_INVALID_ADDRESS < 800);
    assert!(ERR_NETWORK_CONNECTION_NOT_FOUND >= 700 && ERR_NETWORK_CONNECTION_NOT_FOUND < 800);
