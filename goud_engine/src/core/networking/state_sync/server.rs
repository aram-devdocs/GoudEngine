...
impl StateSyncServer {
            ...
            .flat_map(|archetype| archetype.entities().iter().copied())
        {
            ...
            for (component_name, value) in component_values {
                if component_name == type_name::<Transform2D>() {
                    ...
                    {
                        Some(previous) => transform.delta_from(&previous).map(Transform2DSnapshot::Delta),
                ...
                if component_name == type_name::<Transform>() {
                    ...
                    {
                        Some(previous) => transform.delta_from(&previous).map(TransformSnapshot::Delta),
    ...
    fn record_bandwidth(&mut self, kind: MessageKind, payload: &StateSnapshotPayload) -> GoudResult<()> {
...
fn build_full_payload(resolved_entities: &HashMap<Entity, ResolvedSyncEntity>) -> StateSnapshotPayload {
    let mut entities: Vec<StateSyncEntitySnapshot> = resolved_entities
        .values()
        .map(|entity| StateSyncEntitySnapshot {
