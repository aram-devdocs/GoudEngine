//! Nintendo Switch wgpu initialization path.
//!
//! Separated from `init.rs` to stay within the 500-line file limit.
//! TODO(switch-vulkan): Extract shared init logic (bind group layouts, fallback
...
impl WgpuBackend {
    /// Creates a new wgpu backend from a Nintendo Switch window handle.
    ///
    /// Forces the Vulkan backend since Switch uses Vulkan 1.1 (via NVN
    /// translation layer). Blocks on async wgpu initialization via pollster.
    pub fn new_from_switch_handle(
    ...
    async fn new_switch_async(
        ...
        let surface = instance
            ...
            .map_err(|e| GoudError::BackendNotSupported(format!("wgpu Switch surface: {e}")))?;
