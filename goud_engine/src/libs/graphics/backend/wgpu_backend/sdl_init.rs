//! SDL2 wgpu initialization path.
//!
//! Separated from `init.rs` to stay within the 500-line file limit.
//! TODO(sdl-window): Extract shared init logic (bind group layouts, fallback
...
impl WgpuBackend {
    /// Creates a new wgpu backend from an SDL2 window handle.
    ///
    /// Forces the Vulkan backend since SDL2 on Linux uses Vulkan.
    /// Blocks on async wgpu initialization via pollster.
    pub fn new_from_sdl_handle(
        handle: Arc<super::sdl_surface::SdlWindowHandle>,
        width: u32,
        height: u32,
        vsync: bool,
    ) -> GoudResult<Self> {
        pollster::block_on(Self::new_sdl_async(handle, width, height, vsync))
    }

    async fn new_sdl_async(
        handle: Arc<super::sdl_surface::SdlWindowHandle>,
        width: u32,
        height: u32,
        vsync: bool,
        ...
        let surface = instance
            ...
            .map_err(|e| GoudError::BackendNotSupported(format!("wgpu SDL surface: {e}")))?;
        ...
        let adapter = instance
            ...
            .await
            .map_err(|e| {
                GoudError::BackendNotSupported(format!("No suitable Vulkan adapter: {e}"))
            })?;

        let (device, queue): (wgpu::Device, wgpu::Queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("GoudEngine-SDL"),
