...
impl UdpNetProvider {
    ...
    fn handle_recv(&mut self, src: SocketAddr, data: &[u8]) {
        ...
        match header.packet_type {
            PACKET_CONNECT => self.handle_connect(src, &header),
            PACKET_CONNECT_ACK => self.handle_connect_ack(src, &header),
            PACKET_DATA => self.handle_data(src, &header, payload),
            PACKET_DISCONNECT => self.handle_disconnect_packet(src),
            PACKET_HEARTBEAT => self.handle_heartbeat(src, &header),
            _ => {} // Unknown type; drop.
        }
    }

    fn handle_connect(&mut self, src: SocketAddr, header: &PacketHeader) {
        if !self.is_host {
            return;
        }
        ...
        if let Some(conn) = self.connections.get_mut(&id.0) {
            ...
            // Use socket directly to avoid borrow issues with send_raw.
    ...
    fn handle_connect_ack(&mut self, src: SocketAddr, header: &PacketHeader) {
    ...
    fn handle_data(&mut self, src: SocketAddr, header: &PacketHeader, payload: &[u8]) {
        ...
        conn.last_recv = Instant::now();
        conn.stats.bytes_received += (HEADER_SIZE + payload.len()) as u64;
    ...
    fn process_retransmits(&mut self) {
        // Collect retransmit data first to avoid borrow issues.
        ...
        for conn in self.connections.values_mut() {
            ...
            for (_seq, data) in resend_list {
                ...
                to_send.push((conn.addr, packet));
            }
        }

        self.stats.packets_lost += total_lost;
...
impl ProviderLifecycle for UdpNetProvider {
        ...
        let mut incoming: Vec<(SocketAddr, Vec<u8>)> = Vec::new();
        {
            ...
            loop {
                match socket.recv_from(self.recv_buf.as_mut()) {
                    Ok((len, src)) => {
                        incoming.push((src, self.recv_buf[..len].to_vec()));
                    }
    ...
    fn shutdown(&mut self) {
        ...
        for addr in addrs {
            // Build a minimal disconnect header.
...
impl NetworkProvider for UdpNetProvider {
    ...
    fn send(&mut self, conn_id: ConnectionId, channel: Channel, data: &[u8]) -> GoudResult<()> {
        ...
        // Queue for retransmit if reliable (channel 0).
        if channel.0 == 0 {
            conn.reliability.queue_reliable(seq, data.to_vec());
        }

        let addr = conn.addr;
        conn.stats.bytes_sent += packet.len() as u64;
