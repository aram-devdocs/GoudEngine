...
public class GoudGameNetworkMethodGenerationTests
{
    ...
    public void GoudGame_Has_Network_Methods_With_Primitive_Signatures()
    {
        ...
        AssertMethod("NetworkConnectWithPeer", typeof(NetworkConnectResult), typeof(int), typeof(string), typeof(ushort));
        ...
        AssertMethod("NetworkReceivePacket", typeof(NetworkPacket?), typeof(long));
    ...
    public void GoudGame_Network_Methods_Map_To_Expected_Ffi()
    {
        ...
        var generatedContextSource = ReadGeneratedGoudContextSource();
        var nativeMethodsSource = ReadGeneratedNativeMethodsSource();
    ...
    [Fact]
    public void GoudContext_Exposes_Full_Low_Level_Networking_Surface()
    {
        AssertContextMethod("GetNetworkCapabilities", typeof(NetworkCapabilities));
        AssertContextMethod("NetworkHost", typeof(long), typeof(int), typeof(ushort));
        AssertContextMethod("NetworkConnect", typeof(long), typeof(int), typeof(string), typeof(ushort));
        AssertContextMethod("NetworkConnectWithPeer", typeof(NetworkConnectResult), typeof(int), typeof(string), typeof(ushort));
        AssertContextMethod("NetworkDisconnect", typeof(int), typeof(long));
        AssertContextMethod("NetworkSend", typeof(int), typeof(long), typeof(ulong), typeof(byte[]), typeof(byte));
        AssertContextMethod("NetworkReceive", typeof(byte[]), typeof(long));
        AssertContextMethod("NetworkReceivePacket", typeof(NetworkPacket?), typeof(long));
        AssertContextMethod("NetworkPoll", typeof(int), typeof(long));
        AssertContextMethod("GetNetworkStats", typeof(NetworkStats), typeof(long));
        AssertContextMethod("NetworkPeerCount", typeof(int), typeof(long));
        AssertContextMethod("SetNetworkSimulation", typeof(int), typeof(long), typeof(NetworkSimulationConfig));
        AssertContextMethod("ClearNetworkSimulation", typeof(int), typeof(long));
        AssertContextMethod("SetNetworkOverlayHandle", typeof(int), typeof(long));
        AssertContextMethod("ClearNetworkOverlayHandle", typeof(int));
    }
    ...
    private static void AssertContextMethod(string name, Type returnType, params Type[] parameterTypes)
    {
        var method = typeof(GoudContext).GetMethod(name, parameterTypes);
        Assert.NotNull(method);
        Assert.Equal(returnType, method!.ReturnType);
    }
    ...
    private static string ReadGeneratedGoudContextSource([CallerFilePath] string sourceFile = "")
    {
        var repoRoot = Path.GetFullPath(
            Path.Combine(Path.GetDirectoryName(sourceFile)!, "..", "..", "..")
        );
        var generatedPath = Path.Combine(repoRoot, "sdks", "csharp", "generated", "GoudContext.g.cs");
        if (File.Exists(generatedPath))
        {
            return File.ReadAllText(generatedPath);
        }
    ...
    private static string ReadGeneratedNativeMethodsSource([CallerFilePath] string sourceFile = "")
    {
        var repoRoot = Path.GetFullPath(
            Path.Combine(Path.GetDirectoryName(sourceFile)!, "..", "..", "..")
        );
        var generatedPath = Path.Combine(repoRoot, "sdks", "csharp", "generated", "NativeMethods.g.cs");
        if (File.Exists(generatedPath))
        {
            return File.ReadAllText(generatedPath);
        }
