...
class GoudGame:
    ...
    def get_frame_phase_timings(self):
        ...
        return FramePhaseTimings(_out_timings.begin_frame_us, _out_timings.end_frame_us, _out_timings.surface_acquire_us, _out_timings.shadow_pass_us, _out_timings.shadow_build_us, _out_timings.render3d_scene_us, _out_timings.uniform_upload_us, _out_timings.render_pass_us, _out_timings.gpu_submit_us, _out_timings.readback_stall_us, _out_timings.surface_present_us, _out_timings.anim_eval_us, _out_timings.bone_pack_us, _out_timings.bone_upload_us)
    ...
    def set_fps_overlay_corner(self, corner):
        ...
        self._lib.goud_debug_set_fps_overlay_corner(self._ctx, int(corner))
    ...
    def set_debugger_paused(self, paused):
        """Pauses or resumes the route-scoped debugger runtime."""
        self._lib.goud_debugger_set_paused(self._ctx, paused)

    def step_debugger(self, kind, count):
        """Consumes debugger-controlled frame or tick step budget for this route."""
        self._lib.goud_debugger_step(self._ctx, int(kind), count)

    def set_debugger_time_scale(self, scale):
        """Sets the debugger-owned time-scale multiplier for this route."""
        self._lib.goud_debugger_set_time_scale(self._ctx, scale)

    def set_debugger_debug_draw_enabled(self, enabled):
        """Enables or disables runtime-owned debug draw for this route."""
        self._lib.goud_debugger_set_debug_draw_enabled(self._ctx, enabled)

    def inject_debugger_key_event(self, key, pressed):
        """Queues one normalized keyboard event through the debugger control plane."""
        self._lib.goud_debugger_inject_key_event(self._ctx, int(key), pressed)

    def inject_debugger_mouse_button(self, button, pressed):
        """Queues one normalized mouse-button event through the debugger control plane."""
        self._lib.goud_debugger_inject_mouse_button(self._ctx, int(button), pressed)

    def inject_debugger_mouse_position(self, position):
        """Queues one normalized mouse-position event through the debugger control plane."""
        _position_ffi = _ffi_module.FfiVec2()
        _position_ffi.x = position.x
        _position_ffi.y = position.y
        self._lib.goud_debugger_inject_mouse_position(self._ctx, _position_ffi)

    def inject_debugger_scroll(self, delta):
        """Queues one normalized scroll event through the debugger control plane."""
        _delta_ffi = _ffi_module.FfiVec2()
        _delta_ffi.x = delta.x
        _delta_ffi.y = delta.y
        self._lib.goud_debugger_inject_scroll(self._ctx, _delta_ffi)
    ...
    def capture_debugger_frame(self):
        """Captures the current frame plus debugger-owned metadata attachments for this route."""
        _payload = json.loads(_read_string_buffer(
            lambda _buf, _buf_len: self._lib.goud_debugger_capture_frame_json(self._ctx, _buf, _buf_len)
        ))
        return DebuggerCapture(bytes(_payload.get('imagePng', [])), _payload.get('metadataJson', ''), _payload.get('snapshotJson', ''), _payload.get('metricsTraceJson', ''))

    def start_debugger_recording(self):
        """Starts debugger-owned normalized input recording for this route."""
        self._lib.goud_debugger_start_recording(self._ctx)

    def stop_debugger_recording(self):
        """Stops debugger-owned recording and returns the exported replay artifact."""
        _payload = json.loads(_read_string_buffer(
            lambda _buf, _buf_len: self._lib.goud_debugger_stop_recording_json(self._ctx, _buf, _buf_len)
        ))
        return DebuggerReplayArtifact(_payload.get('manifestJson', ''), bytes(_payload.get('data', [])))

    def start_debugger_replay(self, recording):
        """Starts debugger-owned replay using previously exported recording bytes."""
        _recording_buf = (ctypes.c_uint8 * len(recording)).from_buffer_copy(recording)
        self._lib.goud_debugger_start_replay(self._ctx, ctypes.cast(_recording_buf, ctypes.POINTER(ctypes.c_uint8)), len(recording))

    def stop_debugger_replay(self):
        """Stops any active debugger-owned replay session for this route."""
        self._lib.goud_debugger_stop_replay(self._ctx)

    def get_debugger_replay_status_json(self):
        """Returns the current debugger replay status JSON for this route."""
        return _read_string_buffer(
            lambda _buf, _buf_len: self._lib.goud_debugger_get_replay_status_json(self._ctx, _buf, _buf_len)
        )

    def get_debugger_metrics_trace_json(self):
        """Returns the current debugger metrics-trace JSON for this route."""
        return _read_string_buffer(
            lambda _buf, _buf_len: self._lib.goud_debugger_get_metrics_trace_json(self._ctx, _buf, _buf_len)
        )

    def map_action_key(self, action, key):
        """Maps an action name to a key"""
        return self._lib.goud_input_map_action_key(self._ctx, action, int(key))
