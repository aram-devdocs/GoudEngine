...
export class GoudGame implements IGoudGame {
  ...
  drawSpriteRect(texture: number, x: number, y: number, width: number, height: number, rotation: number, srcX: number, srcY: number, srcW: number, srcH: number, color?: IColor, srcMode = 1): boolean {
    const c = color ?? Color.white();
  ...
  drawSpriteBatch(cmds: ISpriteCmd[]): number {
    ...
    return this.native.drawSpriteBatch(nativeCmds);
  }
