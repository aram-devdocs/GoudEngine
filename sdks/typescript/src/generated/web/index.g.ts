...
interface WasmUiManagerHandle {
  ...
  render(): void;
  node_count(): number;
  create_node(component_type: number): number;
  remove_node(node_id: number): number;
  set_parent(child_id: number, parent_id: number): number;
  get_parent(node_id: number): number;
  get_child_count(node_id: number): number;
  get_child_at(node_id: number, index: number): number;
  set_widget(node_id: number, widget_kind: number): number;
  ...
  set_label_text(node_id: number, text: string): number;
  set_button_enabled(node_id: number, enabled: boolean): number;
  set_image_texture_path(node_id: number, path: string): number;
  set_slider(node_id: number, min: number, max: number, value: number, enabled: boolean): number;
  event_count(): number;
...
export class UiManager implements IUiManager {
  ...
  createNode(componentType: number): number { return this.handle.create_node(componentType); }
  removeNode(nodeId: number): number { return this.handle.remove_node(nodeId); }
  setParent(childId: number, parentId: number): number { return this.handle.set_parent(childId, parentId); }
  getParent(nodeId: number): number { return this.handle.get_parent(nodeId); }
  getChildCount(nodeId: number): number { return this.handle.get_child_count(nodeId); }
  getChildAt(nodeId: number, index: number): number { return this.handle.get_child_at(nodeId, index); }
  setWidget(nodeId: number, widgetKind: number): number { return this.handle.set_widget(nodeId, widgetKind); }
  ...
  setLabelText(nodeId: number, text: string): number { return this.handle.set_label_text(nodeId, text); }
  setButtonEnabled(nodeId: number, enabled: boolean): number { return this.handle.set_button_enabled(nodeId, enabled); }
  setImageTexturePath(nodeId: number, path: string): number { return this.handle.set_image_texture_path(nodeId, path); }
  setSlider(nodeId: number, min: number, max: number, value: number, enabled: boolean): number { return this.handle.set_slider(nodeId, min, max, value, enabled); }
  ...
  createPanel(): number { return this.createNode(0); }
  createLabel(text: string): number { const nodeId = this.createNode(2); this.setLabelText(nodeId, text); return nodeId; }
  createButton(enabled = true): number { const nodeId = this.createNode(1); this.setButtonEnabled(nodeId, enabled); return nodeId; }
  createImage(path: string): number { const nodeId = this.createNode(3); this.setImageTexturePath(nodeId, path); return nodeId; }
  createSlider(min: number, max: number, value: number, enabled = true): number { const nodeId = this.createNode(4); this.setSlider(nodeId, min, max, value, enabled); return nodeId; }
}
