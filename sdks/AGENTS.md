...
- `go/` — Go SDK (cgo-based FFI bindings)
- `kotlin/` — Kotlin/JVM SDK (JNI-based FFI bindings)
- `swift/` — Swift SDK (C FFI interop)
- `lua/` — Lua SDK (embedded mlua runtime + LuaRocks distribution)
...
3. Verify matching ctypes declaration in `python/goudengine/generated/_ffi.py`
...
2. Manually update `python/goudengine/generated/_ffi.py` with ctypes signatures
3. Add wrapper methods to C# classes in `csharp/`
4. Add wrapper methods to Python classes in `python/goudengine/`
