...
- `#[ignore]` is permitted only for tests requiring a live GPU context (`test_helpers::init_test_context()`). These tests (OpenGL, audio, sprite batch, render system) cannot run in headless CI. Run them locally with `cargo test -- --ignored`.
